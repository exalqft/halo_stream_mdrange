...
tikzfiles <- hadron::tikz.init("halo_stream_mdrange", width=12, height=6)

for(i in 1:length(archs)) {
  arch <- archs[i]
  bwdat <- dplyr::filter(bws, architecture == arch)
  dat <- read.table(sprintf("%s/results.dat", arch), header=TRUE) %>% dplyr::filter(grepl("4D-complex", field))
  ncol <- NULL
  maxdat <- dplyr::group_by(dat, nt) %>%
            dplyr::filter(n == max(n)) %>%
            dplyr::filter(bw == max(bw)) %>%
            dplyr::ungroup()

  p <- ggplot2::ggplot(dat , aes(x = n^4*16*10^(-6), y = bw, colour = hadron::escapeLatexSpecials(kernel), shape = hadron::escapeLatexSpecials(kernel))) +
       ggplot2::geom_line() +
       ggplot2::geom_point() +
       ...
       ggplot2::facet_wrap(sprintf("$n_{\\textrm{th}}=%03d$",nt) ~ sprintf("field = %s", field), ncol = ncol) +
  ...
  dat <- read.table(sprintf("%s/results.dat", arch), header=TRUE) %>% dplyr::filter(grepl("Gauge-SU3", field))
  ncol <- NULL
  maxdat <- dplyr::group_by(dat, nt) %>%
            dplyr::filter(n == max(n)) %>%
            dplyr::filter(bw == max(bw)) %>%
